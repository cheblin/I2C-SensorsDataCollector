		...
		pub mod Client {
			...
			*Device configuration. Sending from device as reply on PC request, sending from PC as new device configuration */
			...
			pub mod BusConfiguration {
				...
				impl Data_ {
					pub fn multiplier(&mut self) -> crate::util_::Item_V { crate::util_::Item_V(self.0) }
					pub fn time(&mut self) -> crate::util_::Item_f { crate::util_::Item_f(self.0) }
					pub fn clk_khz(&mut self) -> crate::util_::Item_l { crate::util_::Item_l(self.0) }
				}
			...
			*				 it will be proceeded only once */
			...
			pub mod InstructionsPack {
				...
				impl Data_ {
					pub fn Length(&mut self) -> crate::util_::Item_A { crate::util_::Item_A(self.0) }
					pub fn Instructions(&mut self, src: Option<&mut dyn Iterator<Item = u8>>) -> crate::util_::ItemArray_C {
						let dst = self.0;

						let len = 256 as usize;
						let offset = 1;
						let bytes = dst;
						let mut array = crate::util_::ItemArray_C { bytes, len, offset, index: !0 };
						if let Some(src) = src {
							for i in 0..len {
				...
				pub trait DST_ {
					...
					fn Instructions(&mut self, src: &mut crate::util_::ItemArray_C);
				}
			...
			*Bulk sensors data. Sequentially ordered by address sensors data reading */
			...
			pub mod SensorsData {
				...
				impl Data_ {
					pub fn values(&mut self) -> crate::util_::ItemArray_Z {
						let src = self.0;

						let len = 1000 as usize;
						let offset = 0;
						let bytes = src;
						crate::util_::ItemArray_Z { bytes, len, offset, index: !0 }
					}
				}

				pub fn push_<DST: DST_>(src: &mut Data_, dst: &mut DST) { dst.values(&mut src.values()); }
				pub trait DST_ {
					fn values(&mut self, src: &mut crate::util_::ItemArray_Z);
				}
